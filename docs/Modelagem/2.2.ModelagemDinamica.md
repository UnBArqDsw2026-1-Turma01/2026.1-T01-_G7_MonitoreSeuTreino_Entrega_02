...
- **Diagrama de Colaboração**: evidencia a organização estrutural das interações entre os componentes do sistema, destacando a responsabilidade de cada elemento na execução do fluxo.
...
**Figura 1: Diagrama de Sequência - UC01 (Registrar Sessão de Treino). Autor: Samuel Nogueira Caetano.**
...
O diagrama de colaboração representa a interação entre os componentes do sistema durante o caso de uso de registro de sessão de treino, evidenciando a estrutura das interações e a distribuição de responsabilidades entre os elementos.

### 3.2. Diagrama

![Diagrama de Colaboração](../anexos/img/ModelagemDinamica/Diagrama-de-colaboracao.png)

**Figura 2: Diagrama de Colaboração - Sessão de Treino. Autor: Lucas Antunes.**

### 3.3. Descrição do Fluxo

O fluxo inicia com o **Usuário**, que aciona o `SessionController`.

O `SessionController` delega a execução ao `WorkoutService`, responsável pela lógica de negócio.

Durante o processamento:

- Consulta da rotina ativa via `WorkoutRepository`
- Validação dos dados da sessão
- Persistência da sessão
- Atualização de métricas via `MetricsService`

O `SessionPresenter` retorna o resultado ao usuário, podendo indicar sucesso ou erro.

### 3.4. Justificativa

O diagrama de colaboração complementa o diagrama de sequência ao fornecer uma visão estrutural das interações, permitindo identificar dependências e responsabilidades entre componentes.
...
**Figura 3: Diagrama de Atividades - Criação de Rotina Semanal. Autor: Eduardo Waski.**

#### 4.1.3. Descrição do Fluxo
...
**Figura 4: Diagrama de Atividades - Gestão de Exercícios (CRUD). Autor: Giovanni Dornelas.**

#### 4.2.3. Descrição do Fluxo
...
### 4.3. Registrar Sessão de Treino
...
**Figura 5: Diagrama de Atividades - Registrar Sessão de Treino. Autor: Daniel Teles.**

#### 4.3.3. Descrição do Fluxo
...
![Diagrama de Estados - Conta de Usuário](../anexos/img/ModelagemDinamica/diagrama_estados_conta_usuario.png)

**Figura 6: Diagrama de Estados - Conta de Usuário. Autor: José Victor Gabriel Menezes da Costa.**

#### 5.1.3. Justificativas Arquiteturais
...
- **Obrigatoriedade e Isolamento do Onboarding:** Após a criação da conta, a realização do questionário de onboarding se torna obrigatória. A transição estrita do estado `CRIADA` para `ONBOARDING` antes de permitir que a conta alcance o status de `DESLOGADA` amarra visualmente a exigência dos requisitos RF08 e RF09 (Classificação de Perfil).
- **Segurança na Exclusão (RN12):** O diagrama permite que a transição para o estado final `EXCLUÍDA` saia **apenas** do estado `AUTENTICADA` por meio do gatilho de solicitação de exclusão da conta. Isso modela a regra lógica de que o sistema exige um token de sessão válido (verificação de identidade ativa) antes de disparar a remoção definitiva dos dados pessoais.
...
**Figura 7: Diagrama de Estados - Exercício. Autor: André Ricardo Meyer de Melo.**

#### 5.2.3. Descrição do Fluxo
...
### 5.3. Criação de Rotina com Exercícios

#### 5.3.1. Objetivo

O diagrama a seguir ilustra o fluxo de estados envolvido no processo de criação de uma rotina de treino no sistema **G7_MonitoreSeuTreino**, incluindo a busca, seleção e inserção de exercícios, bem como a associação final da rotina à conta do usuário e sua persistência.

#### 5.3.2. Diagrama

![Diagrama de Estados - Criação de Rotina](../anexos/img/ModelagemDinamica/diagrama_estados_mateus.jpg)

**Figura 8: Diagrama de Estados - Criação de Rotina com Exercícios. Autor: Mateus Santos Negrini.**

#### 5.3.3. Descrição do Fluxo

O fluxo se inicia com a ação do usuário ao **buscar ou criar um exercício**, levando o sistema aos estados de **Buscando** ou **Criando**.

No estado **Buscando**, o usuário pode pesquisar exercícios existentes ou explorar novas opções. A partir daí, é possível:

- **Selecionar um exercício** (_Selecionando_), visualizando seus detalhes;
- **Continuar buscando mais exercícios**, retornando ao próprio estado;
- Ou optar por **criar um novo exercício**, transitando para o estado _Criando_ sem perder o contexto da rotina.

No estado **Criando**, um novo exercício é cadastrado, permitindo que ele seja posteriormente utilizado na rotina em construção.

Após a seleção, o sistema transita para o estado **Inserindo**, onde o exercício é adicionado à rotina. Esse processo pode se repetir múltiplas vezes, permitindo a composição completa da rotina com vários exercícios.

Concluída a montagem, o fluxo segue para o estado **Associando**, no qual a rotina criada é vinculada à conta do usuário.

Por fim, o sistema entra no estado **Persistindo**, onde todas as alterações — incluindo a rotina e seus exercícios — são armazenadas no banco de dados, finalizando o processo.

### 5.4. Sessão de Treino

#### 5.4.1. Objetivo

Este diagrama modela o ciclo de vida da Sessão de Treino, isto é, como uma sessão passa por diferentes estados desde sua criação até seu encerramento. Na notação UML, o diagrama de estados é um diagrama que mostra o comportamento discreto de uma parte do sistema por meio de transições finitas (estados). No contexto do projeto, a Sessão é o elemento que melhor representa esse tipo de comportamento porque possui começo, desenvolvimento, pausa, finalização e cancelamento. Além disso, o modelo do sistema prevê que apenas sessões concluídas sejam contabilizadas no histórico e no resumo semanal, enquanto a sessão avulsa continua sendo um cenário válido do domínio.

#### 5.4.2. Diagrama

![Diagrama de Estados - Sessão de treino](../anexos/img/ModelagemDinamica/DiagramaDeEstadoSessao_JoaoNascimento.png)

**Figura 9: Diagrama de Estados - Sessão de Treino. Autor: João Nascimento.**

#### 5.4.3. Descrição

A sessão inicia no estado **Criada**, passa para **Executando** quando o usuário inicia o treino, pode alternar para **Pausado** e retornar para **Executando** quantas vezes forem necessárias, e termina em **Finalizado** ou **Cancelado**. Essa divisão em estados está de acordo com a regra de negócio que diferencia sessão concluída de sessão não concluída, além de permitir representar claramente os caminhos normais e alternativos de uso. A presença de dois estados finais também é coerente com o domínio, porque finalização e cancelamento são desfechos legítimos e distintos.

Os rótulos das transições, como `iniciar()`, `pausar()`, `retomar()`, `finalizar()` e `cancelar()`, representam os eventos que levam o objeto de um estado a outro. Quando necessário, podem ser adicionadas guardas, como `[dadosValidos]`, para mostrar condições de passagem, especialmente na transição para finalização.

O fluxo principal é simples: a sessão é criada, o usuário inicia a execução, a sessão pode ser pausada e retomada, e ao final ela é encerrada com sucesso ou cancelada. Esse comportamento descreve bem o ciclo real de um treino, em que uma execução pode sofrer interrupções, mas ainda assim precisa ser salva de forma confiável quando concluída. O modelo também permite associar o encerramento da sessão ao momento em que os dados passam a estar disponíveis para o histórico e para o resumo semanal.
...
Especificamente, tratando sobre o **Diagrama de Estados: Sessão de Treino**, nota-se como ele é coerente com o diagrama de sequência do registro de sessão, no qual a execução passa por validações, persistência e tratamento de erros. Assim, a máquina de estados funciona como uma síntese do comportamento temporal de uma Sessão, enquanto o diagrama de sequência detalha a interação entre objetos durante a execução desse mesmo fluxo.

## 7. Análise Crítica (Senso Crítico)

### 7.1. Perspectiva do Autor sobre a Geração Automática (Diagrama de Sequência)

**Autor: Samuel Nogueira Caetano**

A elaboração do Diagrama de Sequência (UC01 - Registrar Sessão de Treino) por meio da ferramenta PlantUML não consistiu em uma geração passiva. A abordagem de "Diagrama como Código" foi adotada intencionalmente para garantir que a ordem cronológica e o mapeamento das mensagens (como requisições HTTP e invocações de métodos) refletissem estritamente o fluxo de controle entre as camadas da Arquitetura Limpa. O senso crítico foi aplicado principalmente no desenho das linhas de vida (lifelines) e no isolamento de suas responsabilidades durante a interação: a troca de mensagens direcionada às abstrações (IRotinaRepository e ISessaoRepository) evidencia visualmente que o ExecucaoUseCase orquestra o domínio sem depender de implementações concretas de infraestrutura. O código PlantUML serviu unicamente como motor de renderização para a lógica temporal, os fragmentos combinados para fluxos alternativos e de exceção (blocos alt), e os retornos de validação, elementos que foram ativamente pensados e estruturados por mim para assegurar a integridade do processo antes de qualquer persistência.

### 7.2. Perspectiva Geral da Equipe sobre a Modelagem Dinâmica
...
Já os **Diagramas de Estados** destacaram a simplicidade necessária para o ciclo de vida dessas entidades, refletindo diretamente as ações disponíveis na interface, mostrando como é particularmente útil quando há um ciclo de vida bem definido e um conjunto limitado de transições válidas. O uso de guards (`[dadosValidos]` e `[dadosInvalidos]`) deixou explícitas as pré-condições para cada transição, reforçando a consistência dos dados antes da persistência. Na Conta de Usuário, a modelagem validou a obrigatoriedade do onboarding (RF08/RF09) e a exigência de autenticação para exclusão (RN12); no Exercício, reforçou o fluxo real do protótipo com foco em cadastro, edição e exclusão. Na Sessão de Treino, a principal vantagem dessa modelagem é que ela separa claramente os desfechos da sessão: concluída ou cancelada. Isso melhora a compreensão do sistema e evita ambiguidades sobre o que pode ou não alimentar o histórico e o resumo semanal.
...
| :--------: | :--------: | :-------------------------------------------------------------------------------- | :----------------------------------: | :---------------: |
| 21/04/2026 |    1.0     | Elaboração do script PlantUML e fluxos descritivos.                               |            Samuel Caetano            | Eduardo Valadares |
| 21/04/2026 |    1.1     | Adição das seções 1 a 5                                                           |            Samuel Caetano            | Eduardo Valadares |
| 23/04/2026 |    1.2     | Inclusão do Diagrama de Atividades de Criação de Rotina Semanal e fluxos.         |            Eduardo Waski             |   Lucas Antunes   |
| 23/04/2026 |    1.3     | Inclusão do Diagrama de Atividades de Gestão de Exercícios (CRUD)                 |          Giovanni Dornelas           |   Lucas Antunes   |
| 23/04/2026 |    1.4     | Inclusão do Diagrama de Atividades para o registro de sessão de treino            |             Daniel Teles             |   Lucas Antunes   |
| 23/04/2026 |    1.5     | Adição do Diagrama de Estados do Exercício                                        |             André Meyer              |   Lucas Antunes   |
| 23/04/2026 |    1.6     | Adição do Diagrama de Estados da Conta de Usuário e justificativas arquiteturais  | José Victor Gabriel Menezes da Costa |   Lucas Antunes   |
| 23/04/2026 |    1.7     | Adição do Diagrama de Estados do Exercício                                        |     André Ricardo Meyer de Melo      |   Lucas Antunes   |
| 23/04/2026 |    1.8     | Correção do conflito de merge                                                     |            Lucas Antunes             |  João Nascimento  |
| 23/04/2026 |    1.9     | Adicionado diagrama de estados                                                    |        Mateus Santos Negrini         |   Lucas Antunes   |
| 24/04/2026 |    1.10    | Adição do Diagrama de Estados da Sessão de Treino e o detalhamento desse diagrama |           João Nascimento            |   Lucas Antunes   |
| 24/04/2026 |    1.11    | Adição do Diagrama de Colaboração e ajustes na modelagem dinâmica                 |            Lucas Antunes             |  João Nascimento  |
| 24/04/2026 |    1.12    | Correção do diagrama de estados: Conta de Usuário                                 | José Victor Gabriel Menezes da Costa |   Lucas Antunes   |
| 24/04/2026 |    1.13    | Padronização de legendas, autoria e inclusão de senso crítico personalizado.      |            Samuel Caetano            |         -         |
