...
## 4. Diagrama de Estados: Exercício
...
O **Diagrama de Estados** complementa os demais artefatos ao descrever o ciclo de vida da entidade Exercício, reforçando a coerência com o Diagrama de Classes e com as regras de negócio de integridade já tratadas no Diagrama de Atividades de Gestão de Exercícios (CRUD).
...
Já o **Diagrama de Estados** do Exercício destacou a simplicidade necessária para o ciclo de vida dessa entidade, refletindo diretamente as ações disponíveis na interface. O uso de guards (`[dadosValidos]` e `[dadosInvalidos]`) deixou explícitas as pré-condições para cada transição, reforçando a consistência dos dados antes da persistência.
...
| 23/04/2026 |    1.6     | Correção do conflito de merge                                               | Lucas Antunes     |      -      |
